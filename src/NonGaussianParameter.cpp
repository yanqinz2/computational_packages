...
void NonGaussianParameter::read_commands(int argc, char* argv[])
{
...
void NonGaussianParameter::read_input_file()
{
...
void NonGaussianParameter::initialize_rest_members()
{
...
#pragma omp parallel for
    ...
    mean_quartic_displacement_.resize(number_of_layers_);
    for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
        mean_quartic_displacement_[i_layer].resize(number_of_types_of_atoms_);
        for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
            mean_quartic_displacement_[i_layer][i_type].resize(number_of_timepoints_);
        }
    }

    non_gaussian_parameter_.resize(number_of_layers_);
    for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
        non_gaussian_parameter_[i_layer].resize(number_of_types_of_atoms_);
        for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
            non_gaussian_parameter_[i_layer][i_type].resize(number_of_timepoints_);
        }
    }
    ...
    for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
        for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
            for (int i_timepoint = 0; i_timepoint < number_of_timepoints_; ++i_timepoint) {
                ...
                mean_quartic_displacement_[i_layer][i_type][i_timepoint] = 0.0;
            }
        }
    }
...
void NonGaussianParameter::compute_ngp()
{
    std::cout << "Computing...\n" << std::endl;
...
#pragma omp parallel for
            for (int i_atom = 0; i_atom < number_of_atoms_; ++i_atom) {
                ...
                if (base_atom_layer_ == measurement_atom_layer_ && base_atom_layer_ >= 0 && base_atom_layer_ < number_of_layers_) {
                    ...
                    double temp_add_quartic_, temp_add_square_ = 0.0;
                    if (calculation_dimension_ < 10) {
                        temp_add_square_ = (measurement_atom_coordinate_[i_atom][dimension_0_] - base_atom_coordinate_[i_atom][dimension_0_]) *
                                           (measurement_atom_coordinate_[i_atom][dimension_0_] - base_atom_coordinate_[i_atom][dimension_0_]);
                    }
                    else if (calculation_dimension_ > 100) {
                        for (int i_dimension = 0; i_dimension < dimension_; ++i_dimension) {
                            temp_add_square_ += (measurement_atom_coordinate_[i_atom][i_dimension] - base_atom_coordinate_[i_atom][i_dimension]) *
                                                (measurement_atom_coordinate_[i_atom][i_dimension] - base_atom_coordinate_[i_atom][i_dimension]);
                        }
                    }
                    else {
                        temp_add_square_ = (measurement_atom_coordinate_[i_atom][dimension_1_] - base_atom_coordinate_[i_atom][dimension_1_]) *
                    ...
                    temp_add_quartic_ = temp_add_square_ * temp_add_square_;
#pragma omp atomic                    
                    mean_square_displacement_[base_atom_layer_][temp_atom_type_][i_timepoint - 1] += temp_add_square_;
#pragma omp atomic  
                    mean_quartic_displacement_[base_atom_layer_][temp_atom_type_][i_timepoint - 1] += temp_add_quartic_;
#pragma omp atomic  
    ...
    for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
        for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
            for (int i_timepoint = 0; i_timepoint < number_of_timepoints_; ++i_timepoint) {
                ...
                mean_quartic_displacement_[i_layer][i_type][i_timepoint] /= number_of_atoms_for_average_[i_layer][i_type][i_timepoint];
            }
        }
    }
...
#pragma omp parallel for
        for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
            for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
                for (int i_timepoint = 0; i_timepoint < number_of_timepoints_; ++i_timepoint) {
                    non_gaussian_parameter_[i_layer][i_type][i_timepoint] = mean_quartic_displacement_[i_layer][i_type][i_timepoint] /
                                                                            (mean_square_displacement_[i_layer][i_type][i_timepoint] * mean_square_displacement_[i_layer][i_type][i_timepoint] * 6.0) - 1.0;
                }
            }
        }
    }
...
#pragma omp parallel for
        for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
            for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
                for (int i_timepoint = 0; i_timepoint < number_of_timepoints_; ++i_timepoint) {
                    non_gaussian_parameter_[i_layer][i_type][i_timepoint] = 0.5 * mean_quartic_displacement_[i_layer][i_type][i_timepoint] /
                                                                            (mean_square_displacement_[i_layer][i_type][i_timepoint] * mean_square_displacement_[i_layer][i_type][i_timepoint]) - 1.0;
                }
            }
        }
    }
...
#pragma omp parallel for
        for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
            for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
                for (int i_timepoint = 0; i_timepoint < number_of_timepoints_; ++i_timepoint) {
                    non_gaussian_parameter_[i_layer][i_type][i_timepoint] = 0.6 * mean_quartic_displacement_[i_layer][i_type][i_timepoint] /
                                                                            (mean_square_displacement_[i_layer][i_type][i_timepoint] * mean_square_displacement_[i_layer][i_type][i_timepoint]) - 1.0;
                }
            }
        }
...
void NonGaussianParameter::write_output_file()
{
    std::cout << "Writing output file...\n" << std::endl;

    std::ofstream msd_output_file_(output_file_name_);
    msd_output_file_.precision(output_precision_);

    for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
        if (print_headline_) {
            msd_output_file_ << "Layer range: " << layer_left_point_ + i_layer * layer_width_ << " - "
                             << layer_left_point_ + (i_layer + 1) * layer_width_  << "\n";
            msd_output_file_ << "time";
            for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
                msd_output_file_ << " atom_" << i_type + 1;
            }
            msd_output_file_ << "\n";
        }

        for (int i_timepoint = 0; i_timepoint < number_of_timepoints_; ++i_timepoint) {
            msd_output_file_ << time_table_[i_timepoint];
            for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
                msd_output_file_ << " " << non_gaussian_parameter_[i_layer][i_type][i_timepoint];
            }
            msd_output_file_ << "\n";
        }
    }

    msd_output_file_.close();
}