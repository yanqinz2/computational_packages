...
void MeanSquareDisplacement::initialize_rest_members()
{
    ...
    for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
        mean_square_displacement_[i_layer].resize(number_of_timepoints_);
        for (int i_timepoint = 0; i_timepoint < number_of_timepoints_; ++i_timepoint) {
            mean_square_displacement_[i_layer][i_timepoint].resize(number_of_types_of_atoms_);
        }
    }
...
#pragma omp atomic
                    mean_square_displacement_[atom_layer_[i_average][i_atom]][i_timepoint - 1][atom_type_[i_atom] - 1] += temp_square_distance_;
#pragma omp atomic
...
void MeanSquareDisplacement::write_output_file()
{
    ...
    for (int i_layer = 0; i_layer < number_of_layers_; ++i_layer) {
        ...
        output_file_ << "\n";
        for (int i_timepoint = 0; i_timepoint < number_of_timepoints_; ++i_timepoint){
            output_file_ << (i_timepoint + 1) * interval_ * frame_interval_time_;
            for (int i_type = 0; i_type < number_of_types_of_atoms_; ++i_type) {
